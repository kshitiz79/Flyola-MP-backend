...
async function bookHelicopterSeatsWithoutPayment(req, res) {
    ...
    if (!helicopterSchedule) {
        return res.status(400).json({ success: false, error: `Invalid schedule_id: ${booking.schedule_id} does not exist in helicopter_schedules` });
    }
...
async function cancelHelicopterBooking(req, res) {
    ...
    try {
        ...
        if (!booking.HelicopterSchedule) {
            await t.rollback();
            return res.status(400).json({ error: 'This is not a helicopter booking' });
        }
        ...
        if (!departureTimeRaw || !/^\d{2}:\d{2}:\d{2}$/.test(departureTimeRaw)) {
            await t.rollback();
            return res.status(400).json({ error: 'Invalid departure time format in helicopter schedule. Expected HH:mm:ss.' });
        }
        ...
        if (!departureTime.isValid()) {
            await t.rollback();
            return res.status(400).json({ error: 'Failed to parse departure time in helicopter schedule' });
        }
        ...
        res.status(500).json({ error: 'Failed to cancel helicopter booking: ' + err.message });
    }
}
...
async function rescheduleHelicopterBooking(req, res) {
    ...
    try {
        ...
        if (!newSchedule) {
            await t.rollback();
            return res.status(400).json({ error: 'New helicopter schedule not found' });
        }
        ...
        for (const seat of newSeatLabels) {
            if (!availableSeats.includes(seat)) {
                await t.rollback();
                return res.status(400).json({ error: `Seat ${seat} is not available on new helicopter schedule` });
            }
        }
        ...
        if (t) await t.rollback();
        res.status(500).json({ error: 'Failed to reschedule helicopter booking: ' + err.message });
    }
}
...
async function getHelicopterBookings(req, res) {
    try {
        ...
        res.status(500).json({ error: `Failed to fetch helicopter bookings: ${err.message}` });
    }
}
