...
const addAirport = async (req, res) => {
  const { city, airport_code, airport_name, status = 1 } = req.body;
  ...
  try {
    const airport = await models.Airport.create({ city, airport_code, airport_name, status });
    res.status(201).json({
...
const updateAirport = async (req, res) => {
  ...
  const { city, airport_code, airport_name, status } = req.body;
  if (!city || !airport_code || !airport_name) {
    return res.status(400).json({ error: 'City, airport code, and airport name are required' });
  }
  ...
  try {
    ...
    await airport.update({ city, airport_code, airport_name, status: status ?? airport.status });
    res.json({ message: 'Airport updated successfully', airport });
  } catch (err) {
    console.error('updateAirport error:', err);
...
const deleteAirport = async (req, res) => {
  ...
  try {
    ...
    console.error('deleteAirport error:', err);
...
const activateAllAirports = async (req, res) => {
  try {
    ...
    console.error('activateAllAirports error:', err);
...
const editAllAirports = async (req, res) => {
  ...
  try {
    ...
    console.error('editAllAirports error:', err);
...
const deleteAllAirports = async (req, res) => {
  try {
    ...
    console.error('deleteAllAirports error:', err);
